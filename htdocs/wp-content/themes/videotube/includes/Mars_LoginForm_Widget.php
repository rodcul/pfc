...
class Mars_LoginForm_Widget_Class extends WP_Widget{
	function Mars_LoginForm_Widget_Class(){
		$widget_ops = array( 'classname' => 'mars-loginform-widget', 'description' => __('VT Login/Profile Widget', 'mars') );
		
		$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'mars-loginform-widget' );
		
		$this->WP_Widget( 'mars-loginform-widget', __('VT Login/Profile Widget', 'mars'), $widget_ops, $control_ops );		
	}
	function widget($args, $instance){
		...
		$uploader_url = ( $instance['uploader_url'] != -1 ) ? get_permalink( $instance['uploader_url'] )  : '#';
		$profile_url = ( $instance['profile_url'] != -1 ) ? get_permalink( $instance['profile_url'] )  : '#';
		$html .=  $before_widget;
